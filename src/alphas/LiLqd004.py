# Liquidity 004: Lou and Shu (2017)
#
...
class LiLqd004():
    ...
    def __compute_liquidity(self, x):
        ...
        num1 = abs(x['ret']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
        num2 = (1 / x['daily_volume']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
        LSIlliq = num1 * num2
        return LSIlliq
