...
class ratio_volumeH8():
    def __init__(self, args_dict):
        self.__name = 'ratio_volumeH8'
    ...
    def __compute_liquidity(self, x):
        x['volumeH8'] = pd.to_numeric(x['volumeH8'], errors='coerce')
        x['daily_volume'] = pd.to_numeric(x['daily_volume'], errors='coerce')
        return (-x['volumeH8'] / x['daily_volume']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
