# Liquidity 002: Barra CNE6 Liquidity, avtr
# R = Annualized Traded Value Ratio
# First, the daily traded value ratio (DTVR) is the volume divided by the number 
# of shares. Then, the monthly traded value ratio (MTVR) is the median DTVR 
# on traded days times the number of traded days in last 21 weekdays. The 
# annual traded value ratio (ATVR) is the exponential weighted average of the 
# MTVR in the last 252 days with a half-life of 63 days. The descriptor uses 
# natural logarithm of the computed value. 
...
class LiLqd002():
    ...
    def __compute_liquidity(self, x):
        ...
        turnover_rate = x['daily_volume'] / x['neg_shares']

        # Calculate Monthly Traded Value Ratio (MTVR)
        mtvr = turnover_rate.rolling(window=15, min_periods=15).median() * 15

        # Calculate Annual Traded Value Ratio (ATVR)
        factor = mtvr.ewm(halflife=63, min_periods=self.__window).mean()
        atvr = np.log(factor)
    ...
    def compute(self):
        ...
        dates.sort()
        dates = dates[(self.__window-1+15):]
