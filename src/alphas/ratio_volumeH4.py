...
class ratio_volumeH4():
    def __init__(self, args_dict):
        self.__name = 'ratio_volumeH2'
    ...
    def __compute_liquidity(self, x):
        x['volumeH4'] = pd.to_numeric(x['volumeH4'], errors='coerce')
        x['daily_volume'] = pd.to_numeric(x['daily_volume'], errors='coerce')
        return (-x['volumeH4'] / x['daily_volume']).rolling(self.__window, min_periods=self.__window, closed='left').mean()
