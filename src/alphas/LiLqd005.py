...
class LiLqd005():
    ...
    def __compute_liquidity(self, x):
        ...
        num1 = abs(x['ret'])
        num1ma = num1.rolling(self.__window, min_periods=self.__window, closed='left').mean()
        num2 = (1 / x['daily_volume'])
        num2ma = num2.rolling(self.__window, min_periods=self.__window, closed='left').mean()
        LSIlliq2 = num1ma * num2ma + np.cov(num1, num2)[0, 1]
        return LSIlliq2
